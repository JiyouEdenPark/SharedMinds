class RippleThinking {
    ...
    showThought(text, x = null, y = null) {
        // 랜덤 위치 생성 (입력창 영역 제외)
        const margin = 100;
        const thoughtX =
    ...
    createRipple(thought) {
        ...
        const word =
            (thought.translations && thought.translations[lang]) ||
            thought.text;
        ...
        this.thoughts.forEach((thought) => {
            ...
            if (now > thought.fadeStartTime) {
                ...
                for (let r = 0; r < rings; r++) {
                    ...
                    const ringStart =
                        thought.fadeStartTime +
                        this.elementFadeMs +
                        r * this.ringDelayMs;
                    // 해당 링의 문자들만 추려서 슬롯 인덱스 기준으로 내부 지연 적용
                    const ringChars = thought.ripples.filter(
                        (ch) => ch.ringIndex === r
                    );
                    ringChars.forEach((ch, idx) => {
                        ...
                        if (now >= charStart) {
                            const t = Math.min(
                                ...
                                Math.max(
                                    0,
                                    (now - charStart) / (charEnd - charStart)
                                )
                            );
                            ch.opacity = 1 - t;
                        }
