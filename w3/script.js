class SharedRippleThinking {
  ...
  setupSocketManager() {
    ...
    this.socketManager.onInitialTexts = (texts) => {
      texts.forEach((textObj) => {
        this.showThought(textObj.text, textObj.x, textObj.y, textObj.id);
      });
    };
  ...
  sendThoughtToServer(text) {
    ...
    const sent = this.socketManager.sendText(text, x, y);

    if (!sent) {
      this.showThought(text, x, y);
    }
  }
  ...
  showThought(text, x = null, y = null, id = null) {
    // 랜덤 위치 생성 (입력창 영역 제외)
    const margin = 100;
    const thoughtX =
    ...
    this.thoughts.forEach((thought) => {
      ...
      if (now > thought.fadeStartTime) {
        ...
        if (now >= mainStart) {
          const t = Math.min(
            1,
            Math.max(0, (now - mainStart) / (mainEnd - mainStart))
          );
        ...
        for (let r = 0; r < rings; r++) {
          ...
          const ringStart =
            thought.fadeStartTime + this.elementFadeMs + r * this.ringDelayMs;
