...
function draw() {
    ...
    if (!points) return;
    const size = Math.max(1, Math.min(10, parseInt(ptSize.value||3)));
    const alpha = Math.max(0.1, Math.min(1, parseFloat(ptAlpha.value||0.9)));
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.status === 'ok') {
                ...
                setStatus(`Auto load complete: ${data.found.join(', ')}`);
            } else {
                setStatus('Auto load failed');
            }
        } else {
            setStatus('Auto load request failed');
        }
    } catch (error) {
        console.error('Auto load error:', error);
        setStatus('An error occurred during auto load.');
    }
}
...
setStatus('Click "Auto Load" to start.');
